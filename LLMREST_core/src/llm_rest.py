...
class MALLM:
    def __init__(self, config: Config):
        ...
        if config.saved_data is not None and config.saved_data != "":
...
@click.option('--server', type=str, required=False,
              help='URL of the server, e.g. http://localhost:5000. If not provided, the server will be inferred from the spec file')
@click.option('--token', type=str, required=False, help='The Auth Token of the service')
